...
def parse():
    ...
    try:
        ...
        if entry['op'] == 'GET' and \
            ...
            if cid is not pd.NA:
                continent, country, regions = location.lookup_ip(entry['ip'])
...
def locate_providers():
    if request.is_json:
        ...
        for prov in provs:
            prov['location'] = []
            for maddr in prov['maddrs']:
                ip = providers.extract_ips_from_maddr((prov['peerId'], maddr))
                if ip != '127.0.0.1' and ip != '::1':
                    continent, country, regions = location.lookup_ip(ip)
                    prov['location'].append({'continent': continent, 'country': country, 'regions': list(regions)})
        return provs, 200
